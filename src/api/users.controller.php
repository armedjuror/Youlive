...
const PK = 'id';
const AK = 'email';
const SELECTABLE = ['channel_id', 'id', 'type', 'email', 'name', 'joined_at', 'last_login_at'];
...
if (array_search(PK, INSERT_FIELDS)) {
    ...
    if ($check['status_code']) {
        ...
        if ($request->method === 'GET') {
            ...
            if (isset($request->query['pk'])) {
                $select = $request->connection->prepare("SELECT * FROM " . TABLE . " WHERE " . PK . "=?");
                $select->bind_param(Database::get_type_char($request->query['pk']), $request->query['pk']);
            } elseif (isset($request->query['ak'])) {
                $select = $request->connection->prepare("SELECT * FROM " . TABLE . " WHERE " . AK . "=?");
            ...
            $insert = $request->connection->prepare('INSERT IGNORE INTO ' . TABLE . ' 
                    ...
                    VALUE (
                        '. implode(', ', array_fill(0, count(INSERT_FIELDS), '?')) .'
                    )
                    ');
